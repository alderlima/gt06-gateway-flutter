...
// Channel para minimizar o app
const platform = MethodChannel('com.example.gt06_gateway/navigation');
...
class _ConfigPageState extends State<ConfigPage> with WidgetsBindingObserver {
  ...
  Future<void> _loadConfig() async {
    ...
    traccarPort.text = (p.getInt('traccarPort') ?? 5055).toString(); // Porta padrão OsmAnd
  ...
  void _initService() {
    ...
    service = GT06Service(
      ...
      traccarPort: int.tryParse(traccarPort.text) ?? 5055,
      imei: imei.text.trim(),
  ...
  Future<void> _toggleTraccar() async {
    if (traccarConnected) {
      await service?.disconnectTraccar();
    } else {
      try {
        final p = await SharedPreferences.getInstance();
        await p.setString('traccarHost', traccarHost.text.trim());
        await p.setInt('traccarPort', int.tryParse(traccarPort.text) ?? 5055);
  ...
  // Botão de sair (encerra completamente)
  ...
  // Minimizar app ao pressionar voltar (em vez de fechar)
  Future<bool> _onWillPop() async {
    try {
      ...
      return true; // fallback: permite fechar
    }
  }
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          _inputField("Porta (HTTP)", traccarPort, isNumber: true),
  ...
  Widget _gpsInfo(String label, String value) => Column(
        children: [
          ...
          Text(value,
              style: const TextStyle(
                  color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold, fontFamily: 'monospace')),
        ],
      );
  ...
  Widget _statusTile(String label, bool active, IconData icon) => Container(
        ...
        child: Column(
          children: [
            ...
            Text(label,
                style: TextStyle(fontSize: 9, color: active ? Colors.cyan : Colors.grey[600])),
            Text(active ? "ON" : "OFF",
                style: TextStyle(
                    fontSize: 12,
